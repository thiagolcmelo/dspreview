...
def report_date():
    ...
    try:
        ...
        start_date = parse_date(request.args.get('start_date', default=None, type=None))
        end_date = parse_date(request.args.get('end_date', default=None, type=None))
        
        results = []
        if start_date and end_date:
            results = Report.query.filter(Report.date >= start_date).filter(Report.date <= end_date)
        elif start_date:
